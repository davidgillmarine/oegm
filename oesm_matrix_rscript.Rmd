...
data_all <- data_all %>%
  mutate(study.ctry = ifelse(aid == "375", "Yemen; Indian Ocean Territories; Australia; Chagos - British Indian Ocean Territory; Maldives; Seychelles; Kenya; Tanzania; Mozambique; India; Indonesia; Cambodia; Myanmar; Thailand; Oman; United Arab Emirates; Qatar; Bahrain; Kuwait; Iraq; Iran; Sri Lanka; South Africa; Djibouti; Eritrea; Sudan; Egypt; Israel; Jordan; Saudi Arabia; Comoros; Madagascar; Mauritius; France -Mayotte; France -Reunion",study.ctry))
...
```{r eval=T}
  ...
  #        # study.ctry=gsub("`United States of America (Hawaii)`","United States of America",study.ctry),
  #        # study.ctry=gsub("`United States of America (Palmyra Atoll near Kiribati but owned by United States of America)`","United States of America",study.ctry),
  #        # study.ctry=gsub(".*Mayotte*.","France",study.ctry), # TAKES OUT EVERYTHING IN COLUMN, DOESN'T RESPECT ;
  #         # study.ctry=gsub("`France (Guadeloupe)`","France",study.ctry),
  #         # study.ctry=gsub("`France (Reunion)`","France",study.ctry),
  # ## Want to check back on name changes / the way I'm referring to them
...
ctry_sum <- ctry.all %>% 
  ...
  mutate(admin=gsub(".*Hawaii*.","United States of America",admin), # parenthesis () names - no other way
         ...
         admin=gsub(".*Reunion*.;","France",admin)
           # admin=gsub(".*Chagos*.","British Indian Ocean Territory",admin) might be covered by the earlier one!
           # admin=gsub("United Republic of Tanzania Comoros","United Republic of Tanzania; Comoros",admin),
                # should be fixed in the 300s, just incase
  ) %>%
  filter(!(aid == '705')) %>% # need this here to remove aid 705
  distinct() %>% # no duplicate aids (e.g. France territorie = lumped into France
  group_by(admin) %>% 
