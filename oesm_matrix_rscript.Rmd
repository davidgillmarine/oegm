...
country2 <- country %>%
  ...
  add_row(Country = "British Indian Ocean Territory", Region = "Asia", code = "IOT", Subregion = "Southern Asia")
    # Note here: I picked Asia and Southern Asia because of the proxmity to the Maldives
...
# Countries: Print Country LIST for Stats
ctry_sum2 %>%
  filter(!is.na(pct)) %>%
  # filter(!is.na(admin)) %>%  
      #note: need to check out what this study was that recorded ONLY regions - can we go back and get countries?!
  select(admin,n,pct,Subregion,Region) %>% 
  arrange (desc(n)) %>% # this arranges them in descending order 
  print()
    # note: Indian Ocean territory = issue for region - doesn't have Region
    # neither does Bahamas




# Regions: Print Continents for Stats
region <- ctry_sum2 %>% 
  filter(!is.na(pct)) %>% 
  select(admin,n,pct,Subregion,Region) %>% 
  arrange(desc(Region)) %>% 
...
region <- ctry.all %>% 
  ...
  filter(!is.na(aid)) %>%
  # slice(aid=705) %>% # need this again here to remove aid 705
...
region2 <- country2 %>%   
  full_join(region, by = c("code" = "adm0_a3"), relationship = "many-to-many") %>% 
  select(-Country) %>%
  #select(admin,n,pct,code,Subregion,Region) %>%
  arrange(desc(n))

region2 <- region %>% 
  left_join(country2, by = c("adm0_a3"  = "code"))
