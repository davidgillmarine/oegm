...
# fix data in country column
  ## NOTE: NOW Edited to Match the NEW Map!!! (2024 - ne_countries - scale 10 type countries)
ctry.fix <- data_all %>%
...
# go back and fix any that need to be fixed in initial part of this section!

# join to full world_countries list
ctry_sum1 <- world_countries %>% 
  left_join(ctry_sum,by="admin") %>%
  mutate(n=na.replace(n,0)) 

ctry_sum$admin[!ctry_sum$admin%in%ctry_sum1$admin] # any missing?

#### I MADE IT THIS FAR! only have a few edits: "Bahamas" "Mauritiana" "NA" "Saint Vincent And The Grenadines" "Timor-Leste"
...
## Drew's WORLD MAP CODE (some is above!!!)
...
# Join your data to country shapes
# You will need to have (or create) a column in your data to join to the country shapes from Natural Earth. Ideally, this will match the adm0_a3 field. Sint Maarten, for example, is SXM. You could also try matching to one of the name fields (name, name_long) in Natural Earth instead of the code. But this can get tricky because the names have to match exactly. 
map_data <- left_join(ne_countries, your_data, by = c("adm0_a3" = "your_data_code"))
