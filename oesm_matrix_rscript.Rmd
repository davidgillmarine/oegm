...
Under # Create Heatmap, the "geom_text" line adds text (numbers) on each heat map cell. We #ed that line for the first OEGM paper, since it was only a subsample of the literature, and thus numbers themselves could be misleading. 
...
eco.outcomes <- c("3. Ecological community","2. Population/species", "4. Ecosystem function")
soc.outcomes <- c("1. Knowledge and behavior", "6. Human well-being","7. Governance")
...
out.eco.v.soc <- data_all %>%
  ...
  mutate(eco.out=ifelse(Outcome_cat%in%eco.outcomes,1,0),
         ...
         es.out=ifelse(Outcome_cat=="5. Ecosystem services",1,0)) %>% 
  group_by(aid) %>% 
  summarise(eco.out=max(eco.out),
            soc.out=max(soc.out),
            es.out=max(es.out),
            sum.out=eco.out+soc.out,
            sum.es.out=sum.out+es.out) # %>% 
