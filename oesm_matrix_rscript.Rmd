...
# read in full intervention and outcome lists
int_list<-import(paste0(inputdir,"intervention abbreviations.csv")) #adjusted for '24 name changes
...
- Entry issues: Country entry Change (aid 375), Year Change (1 paper)
...
data_all2 <-data_all2 %>% 
  mutate(Year.of.publication=gsub("2019","2018",Year.of.publication))  # 2019 should be 2018, can blanket bc no other 2019 papers
```
...
my_heat_map(filter(data_all2,coral==1),"Intervention.subcategory", "Outcome_cat", high.col = "coral3")
io_counts.subint <- io_counts
...
my_heat_map(filter(data_all2,coral==1),"Int_cat", "Outcome.subcategory", high.col = "coral3")
io_counts.subint <- io_counts
(heat.map.subint <- heat.map + labs(x="Conservation Intervention", y="Outcome", title ="Coral"))
...
my_heat_map(filter(data_all2,coral==1),"Intervention.subcategory", "Outcome.subcategory", high.col = "coral3")
io_counts.subintout <- io_counts
(heat.map.subint <- heat.map + labs(x="Conservation Intervention", y="Outcome", title ="Coral"))
...
outcomes_tidy <- outcomes2 %>% 
  ...
  mutate(fill = Outcome_cat,
      ...
      fill=gsub("6. Knowledge & Behavior", "#D55E00",fill), #Vermillion Okabe-Ito
...
(pInt_time_gps <- ggplot(Int_time_gps)  + 
    ...
    scale_linetype_manual(values=c(      
      ...
      "4. Enforcement & Prosecution" = "solid",
    ...
    scale_color_manual(values=c(      # uses Okabe-Ito colors - color-blind friendly!
      ...
      "2. Species Management" = "#56B4E9",   #Sky Blue Okabe-Ito
      "3. Awareness Raising" = "#009E73",    #Bluish Green Okabe-Ito
      "4. Enforcement & Prosecution" = "#F0E442",    # Yellow Okabe-Ito
      "5. Livelihood, Economic, & Other Incentives" = "#E69F00",  #Orange Okabe-Ito
      "6. Conservation Designation & Planning" = "#D55E00", #Vermillion Okabe-Ito
      "7. Legal & Policy Frameworks" =  "#BE0032", # from kelly by grafify - good on color-blindness.com
      "8. Education & Training" = '#882255', #wine - extra color by DIG - good on color-blindness.com
...
# Potential Extra Colors
        ...
        # # kelly by grafify - has mroe colors
...
(pOut_time_gps <- ggplot(out_time_gps)  + 
    ...
    scale_color_manual(values=c(      # uses Okabe-Ito colors - color-blind friendly!
      ...
      "6. Knowledge & Behavior" = "#D55E00", #Vermillion Okabe-Ito
...
plot_grid(pInt_time_gps,pOut_time_gps,
          labels=letters[1:2], ncol = 2, nrow = 1, 
          hjust=-1, align = "hv") #column labels a and b
ggsave(paste0(plotdir,today.date,'_studies_over_time.png'),width = 12,height = 4)

#slighly more zoomed out map - going to use this one in pub!
plot_grid(pInt_time_gps,pOut_time_gps,
          labels=letters[1:2], ncol = 2, nrow = 1,
          hjust=-1, align = "hv")
ggsave(paste0(plotdir,today.date,'_studies_over_time2.png'),width = 16,height = 6)
...
# Studies Over Time Stats! - PERCENT CHANGE PER YEAR
Measuring the change in PERCENT of TOTAL studies PER YEAR

Stats - growth by 5-year increments (half-decades)(called demi-decades)

This is ADDITIVE (number of studies per year) so we don't have to do the "max" command

```{r eval=T}
#PERCENT of total CHANGE PER YEAR

#1994-2000 (aka through 1999)
  ##DON'T USE, USE 1995 below
((All_time_gps$val[All_time_gps$Year.of.publication==1999])-(All_time_gps$val[All_time_gps$Year.of.publication==1994]))/num.studies/6*100

#1995-2000 (aka through 1999)
((All_time_gps$val[All_time_gps$Year.of.publication==1999])-(All_time_gps$val[All_time_gps$Year.of.publication==1995]))/num.studies/5*100

#1990-2000 DECADE (aka through 1999)
  ## Just diving by 10 instead of 6, bc 1990-1993 years were 0!
((All_time_gps$val[All_time_gps$Year.of.publication==1999])-(All_time_gps$val[All_time_gps$Year.of.publication==1994]))/num.studies/10*100

#2000-2005 (aka through 2004)
((All_time_gps$val[All_time_gps$Year.of.publication==2004])-(All_time_gps$val[All_time_gps$Year.of.publication==2000]))/num.studies/5*100

#2005-2010 (aka through 2009)
((All_time_gps$val[All_time_gps$Year.of.publication==2009])-(All_time_gps$val[All_time_gps$Year.of.publication==2005]))/num.studies/5*100

#2000-2010 DECADE (aka through 2009)
((All_time_gps$val[All_time_gps$Year.of.publication==2009])-(All_time_gps$val[All_time_gps$Year.of.publication==2000]))/num.studies/10*100

#2010-2015 (aka through 2014)
((All_time_gps$val[All_time_gps$Year.of.publication==2014])-(All_time_gps$val[All_time_gps$Year.of.publication==2010]))/num.studies/5*100

#2015 THROUGH 2018 (aka 4 years)
((All_time_gps$val[All_time_gps$Year.of.publication==2018])-(All_time_gps$val[All_time_gps$Year.of.publication==2015]))/num.studies/4*100

#2010 THROUGH 2018 - ALMOST DECADE - (aka 9 years)
((All_time_gps$val[All_time_gps$Year.of.publication==2018])-(All_time_gps$val[All_time_gps$Year.of.publication==2010]))/num.studies/9*100

#2015 THROUGH 2018 (aka 4 years) - TOTAL PERCENT OF STUDIES (aka NOT Percent per year, different!)
((All_time_gps$val[All_time_gps$Year.of.publication==2018])-(All_time_gps$val[All_time_gps$Year.of.publication==2015]))/num.studies*100
...
# FOR THE STUDIES WITH 3+ COUNTRIES / TERRITORIES, WHERE are they LOCATED?
  ## Sadaf's Question on the final paper!!!

#Get the AIDs of all studies that have 3+ countries!
multi.ctry.aid <- ctry_sum$aid[ctry_sum$X=="ctry3"]
  # print(multi.ctry.aid)

#now, separate just these papers
ctry_3_plus <- ctry_sum.25 %>%
  filter(aid %in% multi.ctry.aid) %>%
  arrange(aid)

#GET REGION and SUBREGION LIST
# join ctry_three_plus with the regions and continents
ctry_3_plus2 <- ctry_3_plus %>%   
  full_join(ctry_sum2, by = c("admin" = "admin")) %>% 
  select(aid,admin,Subregion,Region,code) %>%
  filter(!is.na(aid)) %>%
  arrange(aid,Subregion,Region)
#write.csv(ctry_3_plus2,paste0(tabledir,today.date,"_ctry_3_plus_studies.csv"))

# GOING OFF OF THAT, what about ALL Multi-national studies?! AKA THE 2+

#Get the AIDs of all studies that have 3+ countries!
multi.ctry.aid2 <- ctry_sum$aid[ctry_sum$X=="ctry2"]
  # print(multi.ctry.aid2)

#now, separate just these papers
ctry_2_plus <- ctry_sum.25 %>%
  filter(aid %in% multi.ctry.aid2) %>%
  arrange(aid)

#GET REGION and SUBREGION LIST
# join with the regions and continents
ctry_2_plus2 <- ctry_2_plus %>%   
  full_join(ctry_sum2, by = c("admin" = "admin")) %>% 
  select(aid,admin,Subregion,Region,code) %>%
  filter(!is.na(aid)) %>%
  arrange(aid,Subregion,Region)
# write.csv(ctry_2_plus2,paste0(tabledir,today.date,"_ctry_2_plus_studies.csv"))
```
...
# How many studies took place in multiple habitats? (2 or more) where CORAL REEFS were 1 Habitat?!
  ## Claudia's question on the final paper
data_all2 %>% 
  filter(coral + seagrass == 2 | coral + mangrove == 2) %>% 
  summarise(habitat="Multiple: 2 or more W CORAL", num=n_distinct(aid), pct=num/num.studies*100)
...
data_all2 %>%  
  filter(Int_cat== "3. Awareness Raising "| 
           Int_cat== "4. Enforcement & Prosecution"| 
...
num.multi.int.studies <- n_distinct(multi.int.studies$aid)
...
Multi_int_time_gps <- data_all2 %>%
  ...
  summarise(pub.per.yr= n_distinct(aid)) %>% 
  ungroup() %>% 
  add_row(Year.of.publication = "1999", pub.per.yr = 0) %>% # a numerical value DOESN'T have "" around it! 
  add_row(Year.of.publication = "2004", pub.per.yr = 0) %>% 
      # Note here: there were no 1999 or 2004 Papers, so I needed to add an entry with 0. 
      # There are other years missing, but I only need to add years I'm doing MATH with, since I'm not graphing this... could graph in supp tho!
  arrange(Year.of.publication) %>% 
  mutate(val=cumsum(pub.per.yr),
...
#######################################################################
#MULTI-INTERVENTION STUDIES: PERCENT of total CHANGE PER YEAR

#1996-2000 (aka through 1999)
  ## 1996 was first year!
((Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==1999])-(Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==1996]))/num.studies/6*100

#1990-2000 DECADE (aka through 1999)
  ## Just diving by 10 instead of 6, bc 1990-1993 years were 0!
((Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==1999])-(Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==1996]))/num.studies/10*100

#2000-2005 (aka through 2004)
((Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2004])-(Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2000]))/num.studies/5*100

#2005-2010 (aka through 2009)
((Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2009])-(Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2005]))/num.studies/5*100

#2000-2010 DECADE (aka through 2009)
((Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2009])-(Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2000]))/num.studies/10*100

#2010-2015 (aka through 2014)
((Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2014])-(Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2010]))/num.studies/5*100

#2015 THROUGH 2018 (aka 4 years)
((Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2018])-(Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2015]))/num.studies/4*100

#2010 THROUGH 2018 - ALMOST DECADE - (aka 9 years)
((Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2018])-(Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2010]))/num.studies/9*100

#2015 THROUGH 2018 (aka 4 years) - TOTAL PERCENT OF STUDIES (aka NOT Percent per year, different!)
((Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2018])-(Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2015]))/num.studies*100

#2015 THROUGH 2018 (aka 4 years) - TOTAL PERCENT OF *MULTI INTERVETION* STUDIES (aka NOT Percent per year, different!)
((Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2018])-(Multi_int_time_gps$val[Multi_int_time_gps$Year.of.publication==2015]))/num.multi.int.studies*100
...
#################################################
#MULTIPLE INTERVENTION STUDIES - UPDATED for EDITS 7/2/24 - DAVID's SCRIPT

# get variables we want
typology_dat2 <- data_all2 %>% 
  select(aid, Int_cat) %>% 
  filter(Int_cat!="" & !is.na(Int_cat)) %>%  # just in case
  distinct() # only unique interventions per aid  
unique(typology_dat2$Int_cat)

# select only multi-intervention studies
multi.int.studies2 <- typology_dat2 %>% 
  group_by(aid) %>% 
  filter(n()!=1) %>%  # only multi-intervention studies
  arrange(aid, Int_cat)  # very impt so as not to get reverse pairs
  
# all intervention combination pairs by study
int.combos.by.aid <- multi.int.studies2 %>%
  group_by(aid) %>%
  do(as_tibble(t(combn(.$Int_cat, m = 2)))) %>% # SHOULD give us all possible pairs by group (need to manually check)
  ungroup() %>%
  mutate(int.pair=paste(V1,V2,sep=": ")) # create variable with pairs
int.combos.by.aid %>% view  # check output to see if this is correct, e.g. interventions in order (V1<V2)

# count all intervention combination pairs across all studies
int.combos <- int.combos.by.aid %>% 
  group_by(int.pair,V1,V2) %>% 
  count %>% 
  arrange(-n) %>%
  mutate(pct.total=n/num.studies*100) %>%   #"pct" column - do we want it out of TOTAL studies? Not right now
  mutate(pct.multi_int=n/n_distinct(multi.int.studies$aid)*100)  %>%  #"pct" column - out of MULTI-INT Studies - 62 studies!
  view()

```

# MULTIPLE intervention visuals! - CHORD DIAGRAM

"Int_cat" for both

```{r eval=T} 
#--- Chord diagram
int.combos2 <- int.combos  %>%
  ungroup() %>%
  select(-c(int.pair,pct.total,n)) %>%
  group_by(V1,V2)

# original
pacman::p_load(circlize)
chordDiagram(int.combos2)
circos.clear()

mycolor <- (values=c(      # uses Okabe-Ito colors - color-blind friendly!
    "1. Land/Water Management" = "#0072B2", #blue Okabe-Ito
    "2. Species Management" = "#56B4E9",   #Sky Blue Okabe-Ito
    "3. Awareness Raising" = "#009E73",    #Bluish Green Okabe-Ito
    "4. Enforcement & Prosecution" = "#F0E442",    # Yellow Okabe-Ito
...
#chord
png(paste0(plotdir,today.date,'_multi-intervention_chord.png'), width = 10, height = 8, units = "in", res=600) 
chordDiagram(int.combos2, grid.col = mycolor, annotationTrack = "grid",
    preAllocateTracks = list(track.height = max(strwidth(unlist(dimnames(int.combos2))))))
circos.track(track.index = 1, panel.fun = function(x, y) {
    circos.text(CELL_META$xcenter, CELL_META$ylim[1], CELL_META$sector.index, 
        facing = "inside", niceFacing = TRUE, adj = c(0, 0.5))
}, bg.border = NA) 
dev.off()

# reverse chord
png(paste0(plotdir,today.date,'_multi-intervention_chord_reverse.png'), width = 10, height = 8, units = "in", res=600) 
chordDiagram(select(int.combos2,V2,V1,pct.multi_int), grid.col = mycolor, annotationTrack = "grid", 
    preAllocateTracks = list(track.height = max(strwidth(unlist(dimnames(int.combos2))))))
circos.track(track.index = 1, panel.fun = function(x, y) {
    circos.text(CELL_META$xcenter, CELL_META$ylim[1], CELL_META$sector.index, 
        facing = "inside", niceFacing = TRUE, adj = c(0, 0.5))
...
data_all2 %>%  
  filter(Outcome_cat== "5. Human Wellbeing"| 
          Outcome_cat== "6. Knowledge & Behavior"| 
...
soc.outcomes <- c("6. Knowledge & Behavior", "5. Human Wellbeing", "7. Governance")
...
out.x.count <- data_all2 %>% 
  ...
  out.x.list <- expand.grid(out_list$out_val,out_list2$out_val)  %>% 
