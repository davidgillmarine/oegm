...
###NOTE: this is MESSY - should try to make world map into a FUNCTIOn like gap map if I want habitat specific maps!
...
ctry_sum_coral <- ctry.coral %>% 
  ...
  filter(!(aid == '705')) %>% # need this here to remove aid 705
  distinct() %>% # no duplicate aids (e.g. France territories = lumped into France
  group_by(admin) 

ctry_sum_coral.5 <- ctry_sum_coral %>%
  count() %>%     #count creates an "n" column
  mutate(pct=round(n/num.studies*100)) %>%   #this creates the "pct" column
  arrange(desc(n))

# Countries that need to be fixed
ctry_sum_coral.5$admin[!ctry_sum_coral.5$admin%in%world_countries$admin]
...
# HOW MANY STUDIES have more than one country Join the Dataframe to rnaturalearth ne_countries (world_countries) 
...
n_distinct(multi.int.studies$aid)/n_distinct(data_all2$aid)*100
