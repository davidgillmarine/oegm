...
pacman::p_load(rmarkdown, broom, rio,treemap,gtools,cowplot,rnaturalearth,rnaturalearthdata,tidyverse,dplyr)
...
# join to full world_countries country shapes (from ne_countries)
  # My column in my data that joins to the country shapes from Natural Earth is the admin field. This can get tricky because the names have to match exactly (which is why I did all that work above with name updates)
ctry_sum1 <- world_countries %>% 
  left_join(ctry_sum,by="admin") %>%
  mutate(n=na.replace(n,0)) 
        # this SHOULD give me a column for n... but it's NOT doing that?!
...
# Map the World!- from Drew
# Load and process your data 
your_data <- 

# Join your data to country shapes
map_data <- left_join(world_countries, ctry_sum, by = "admin")
...
(study_ctry_map <- ggplot() +
    geom_map(data=ctry_sum1, aes(map_id=adm0_a3, fill=n),map=ne_countries()) + #adm0_a3 = the country code in ne_countries 
            #ne_countries or world_country
    expand_limits(x=ne_countries$long,y=ne_countries$lat) +
    theme(
