...
# MORE Data Cleaning, For Categories Later - Year Change, Name Change!
...
(pInt_time_gps <- ggplot(Int_time_gps)  + 
    ...
    scale_color_manual(values=c(      # uses Okabe-Ito colors - color-blind friendly!
      ...
      "7. Legal & Policy Frameworks" =  "#BE0032", # from kelly by grafify
      "8. Education & Training" = "#CC79A7", #reddish purple Okabe-Ito
      "9. Institutional/Organizational Development" = "#604E97" #lighter purple - extra color by DIG
        ))
          # "#999999",    # Grey Okabe-Ito
          # '#332288', #indigo - extra color by DIG
          # "#DCD300" #Yellowgreen that may be easier to see?
          # "#882D17" #Red/brown/orange
          # '#882255', #wine - extra color by DIG
          # 
...
ggsave(paste0(plotdir,today.date,'_Interventions_over_time.png'),width = 12,height = 12)
...
ggsave(paste0(plotdir,today.date,'_studies_over_time.png'),width = 12,height = 6)

plot_grid(pInt_time_gps, pOut_time_gps, ncol = 1, nrow = 2)
ggsave(paste0(plotdir,today.date,'_studies_over_time_2rows.png'),width = 12,height = 12)
```
...
##Avg Count: Studies per year
(max(All_time_gps$val)-min(All_time_gps$val))/(max(All_time_gps$Year.of.publication)-min(All_time_gps$Year.of.publication))
### Avg %: Studies per year
(max(All_time_gps$val)-min(All_time_gps$val))/(max(All_time_gps$Year.of.publication)-min(All_time_gps$Year.of.publication))/num.studies*100

##late 1990s Count - starts in 1994 bc this is the first year
(max(All_time_gps$val[All_time_gps$Year.of.publication<2000])-max(All_time_gps$val[All_time_gps$Year.of.publication<1994]))/6
##late 1990s PERCEMT - starts in 1994 bc this is the first year
(max(All_time_gps$val[All_time_gps$Year.of.publication<2000])-max(All_time_gps$val[All_time_gps$Year.of.publication<1994]))/6/num.studies*100
