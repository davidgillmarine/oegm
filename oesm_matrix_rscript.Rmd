...
all.data<-import(paste0(inputdir,"20231115_All_Papers.xlsx"), which="FULL Data Extraction Sheet",skip =2, .name_repair = "universal") %>% 
  mutate(Date=as.character(Date))

country <- import(paste0(inputdir,"allcountries.csv"))

# Count total papers in Full Text Screening
length(unique(all.data$Article.ID)) #should be 

# For IN PROCESS papers only - Select accepted papers, rename variables==
Incomplete_data <- all.data %>%  
  filter(Full.text.screening.=="Incomplete") %>% 
  rename(Int_cat=Intervention.category,Outcome_cat=Outcome.category, aid=Article.ID, study.ctry=Country.ies..of.study)
length(unique(Incomplete_data$aid)) #= how many incomplete papers left to finish!
...
data_all <- all.data %>%  
  ...
  rename(Int_cat=Intervention.category,Outcome_cat=Outcome.category, aid=Article.ID, study.ctry=Country.ies..of.study)
length(unique(data_all$aid)) # = how many completely screened papers
...
drop.down.lists<-import(paste0(inputdir,"20231115_All_Papers.xlsx"), which="Dropdowns", skip =1, .name_repair = "universal")
...
data_all %>%  
  ...
  summarise(intervention="Highest 2", num=n_distinct(aid), pct=num/num.studies*100)
