...
#AID 375 - Issue with Bad Drop Down / Wrongly coded countries - must mutate
    ...
    # Country names matched with study list taken from study (each place only entered once)
      ...
      # Indian Ocean Territories; Australia; 
          # 2. Western Australia (AUSW: Scott Reef, Cocos–Keeling Islands, Christmas Island), 
      # Chagos (British Indian Ocean Territory); Maldives; Seychelles; 
          # 4. Central Indian Ocean Islands (CIOI: Chagos, Lakshadweep (India), Maldives, Seychelles),
          # NOTE - see note in country names about Chagos
      # Kenya; United Republic of Tanzania; Mozambique; 
          # 5. East Africa (EAFR: Kenya, Tanzania), 
      # India; Indonesia; Cambodia; Myanmar; Thailand; 
          # 6. Eastern Indian Ocean (EIO: Andaman–Nicobar (India), Western Indonesia (Sumatra), Kambodia, Myanmar/Burma, Thailand), 
      # Oman; United Arab Emirates; Qatar; Bahrain; Kuwait; Iraq; Iran; 
          # 7. Arabian/Persian Gulf (GULF), 
      # Sri Lanka; 
          # 8. Southern India and Sri Lanka (IND–SRI), 
      # South Africa; 
          # 9. Mozambique and South Africa (MOZ–SAF), 
      # Djibouti; Eritrea; Sudan; Egypt; Isreal; Jordan; Saudi Arabia; 
          # 10. Red Sea–Gulf of Aden (RS–GA), 
      # Comoros; Madagascar; Mauritius; France (Mayotte); France (Reunion); 
          # 11. South Western Indian Ocean Islands (SWIOI: Comoros, Madagascar, Mauritius, Mayotte, Reunion, Rodrigues (Mauritius)). 
data_all <- data_all %>%
  mutate(study.ctry = ifelse(aid == "375", "Yemen; Indian Ocean Territories; Australia; Chagos - British Indian Ocean Territory; Maldives; Seychelles; Kenya; Tanzania; Mozambique; India; Indonesia; Cambodia; Myanmar; Thailand; Oman; United Arab Emirates; Qatar; Bahrain; Kuwait; Iraq; Iran; Sri Lanka; South Africa; Djibouti; Eritrea; Sudan; Egypt; Isreal; Jordan; Saudi Arabia; Comoros; Madagascar; Mauritius; France -Mayotte; France -Reunion",study.ctry))
...
data_all <-data_all %>% 
  mutate(Int_cat=gsub("1. Land/water management","1. Land/Water Management",Int_cat), 
    ...
    Int_cat=gsub("4. Enforcement & protection","4. Enforcement & Protection",Int_cat),
...
data_all <- data_all %>%
  mutate(study.ctry=gsub(",",";",study.ctry),  # replace "," with ";" to help with splitting
         ...
         study.ctry=gsub("U.S. United States Virgin Islands","United States Virgin Islands",study.ctry),
         study.ctry=gsub("British United States Virgin Islands","British Virgin Islands",study.ctry),
         study.ctry=gsub("Viet Nam","Vietnam",study.ctry),
           # study.ctry=gsub("Comoros",";Comoros",study.ctry), # somehow lost its divider
         ## Would Like a better Name / Map / Disputed... come back to
                # study.ctry=gsub("Chagos*.","British Indian Ocean Territory",study.ctry), #should change all Chagos
         study.ctry=gsub("Chagos - British Indian Ocean Territory","British Indian Ocean Territory",study.ctry,fixed = TRUE),
            # study.ctry=gsub("United States of America*.","United States of America",study.ctry),
         study.ctry=gsub("`United States of America (Hawaii)`","United States of America",study.ctry,fixed = TRUE),
         study.ctry=gsub("`United States of America (Palmyra Atoll near Kiribati but owned by United States of America)`","United States of America",study.ctry,fixed = TRUE),
             # study.ctry=gsub(".*Mayotte*.","France",study.ctry), # TAKES OUT EVERYTHING IN COLUMN, DOESN'T RESPECT ;
          study.ctry=gsub("`France (Guadeloupe)`","France",study.ctry,fixed = TRUE),
          study.ctry=gsub("`France (Mayotte)`","France",study.ctry,fixed = TRUE),
          study.ctry=gsub("`France (Reunion)`","France",study.ctry,fixed = TRUE),
...
(study_ctry_map <- ggplot(data=ctry_sum3) +
  ...
  theme(
        ...
        legend.key.width = unit(1, "null"), # making the legend wider, but no way to make it slightly smaller?
        text=element_text(family="sans") # "serif"=TN Roman, "sans" = Arial
        ) +
  scale_fill_gradient2(
    name = "Percent of Total Studies",
...
ggsave(paste0(plotdir,today.date,'_study_country_map.png'),width = 9,height = 4.5) #was 4.75 - some extra
```

# World Map - Coral

###NOTE: WILL NEED TO BE ADJUSTED!!! I made updates to the world map since :)
...
#NOTE: REQUIRES treemapify package, ggforce package, AND colorspace package - NEED TO RE-ADD TO THE TOP OF THE SCRIPT
...
# HOW MANY STUDIES have more than one coutry Join the Dataframe to rnaturalearth ne_countries (world_countries) 
num_countries <- ctry.all %>% 
  separate(study.ctry,max.ctry, sep="; ",fill="right") %>% # fill=right fills blanks with NAs
  gather("X","admin", max.ctry) %>%   #gather is a tidyr function that reshapes columns into key pairs 
      #renames "study.ctry" to "admin"
  filter(!is.na(admin)) %>% 
...
max.comp <- paste0("comp",rep(1:max(str_count(data_all$Comparator.type, ";")+1))) 
...
## get no. articles per
max.groups <- paste0("comp",rep(1:max(str_count(data_all$If.so..what.types.of.different.groups., ";")+1))) 

## now, get human groups
human.groups <- data_all %>% 
...
#view(io_counts.all)
io.counts.pct <- io_counts.all %>%
  mutate(pct=n/num.studies*100)%>% 
  arrange (desc(n)) %>% # this arranges them in descending order 
  view()
head(io.counts.pct)
tail(io.counts.pct)
...
io.counts.pct.sub <- io_counts.subintout %>%
  mutate(pct=n/num.studies*100)%>% 
  arrange (desc(n)) %>% # this arranges them in descending order 
  view()
head(io.counts.pct.sub)
tail(io.counts.pct.sub)
```
