...
all.data<-import(paste0(inputdir,"20240314_All_Papers.xlsx"), which="FULL Data Extraction Sheet",skip =2, .name_repair = "universal") %>% 
  mutate(Date=as.character(Date))

country <- import(paste0(inputdir,"allcountries.csv"))

# Count total papers in Full Text Screening
length(unique(all.data$Article.ID)) # should be 779
...
length(unique(Reject_data$aid)) #= how many rejected papers # should be 607
...
data_all <- all.data %>%  
  ...
  rename(Int_cat=Intervention.category,Outcome_cat=Outcome.category, aid=Article.ID, study.ctry=Country.ies..of.study)
length(unique(data_all$aid)) # = how many accepted & completely screened papers # should be 172
...
drop.down.lists<-import(paste0(inputdir,"20240314_All_Papers.xlsx"), which="Dropdowns", skip =1, .name_repair = "universal")
...
# CPUE Analysis - Needs to happen to decide what to do with category 1c. Catch/Yield
...
View(Behavior_out)
#write.csv(Behavior_out,paste0(tabledir,today.date,"_1e_outcomes.csv"))

# Now going to Get EVERY Row of data for each of the aids!
Behavior_out2 <- Behavior_out %>% 
  select(aid) %>%
  distinct(aid)
View(Behavior_out2)

Behavior_out_ALL <- 
  merge(Behavior_out2,data_all,sort=TRUE)
View(Behavior_out_ALL)
write.csv(Behavior_out_ALL,paste0(tabledir,today.date,"_1e_outcomes_ALL_ROWS.csv"))


## Outcome 1k. Body Conditions. and 1l. Adaptability - Coral studies and issues!
...
# Now going to Get EVERY Row of data for each of the aids!
Coral_issues2 <- Coral_issues %>% 
  select(aid) %>%
  distinct(aid)
View(Coral_issues2)

Coral_issues_ALL <- 
  merge(Coral_issues2,data_all,sort=TRUE)
View(Coral_issues_ALL)
write.csv(Coral_issues_ALL,paste0(tabledir,today.date,"_1kand1l_outcomes_ALL_ROWS.csv"))

#SPECIES DIVERSITY ISSUES!!!
## For Species Diversity, I will be pulling out SPECIFIC AIDs First (the "No" Examples)
...
## FIREST I'm going to pull out all the CODED AIDs, so I don't have to search through those
...
#### NEXT, I will want to pull out the SOCIAL ONLY papers, and get all of their AIDS, So that I DON'T have to check them for species diversity LOL
#NOTE: I've already gone through the first 100 by hand, easy to go through the rest like that in 1. Completes - Move accordingly


## Pull out KNOWLEDGE (6a)!
know_out <- data_all %>% 
  filter(Outcome.subcategory=="6a. Knowledge")
View(know_out)
# write.csv(know_out,paste0(tabledir,today.date,"_6a_knowledge.csv"))

## Pull out Behavior/Response to new practice (6b)!
behave_out <- data_all %>% 
...
## Pull out Culture(5h) - also looking for WTP!
...
## Pull out 2e. Species Interactions
interactions_out <- data_all %>% 
  filter(Outcome.subcategory=="2e. Species Interactions")
View(interactions_out)
write.csv(interactions_out,paste0(tabledir,today.date,"_2e.csv"))

## Pull out 3b
herbpred_out <- data_all %>% 
  filter(Outcome.subcategory=="3b. Herbivory and Predation")
View(herbpred_out)
write.csv(herbpred_out,paste0(tabledir,today.date,"_3b.csv"))

## Pull out 1g. Reproduction - to check to things that are also behavior...
reprod_out <- data_all %>% 
  filter(Outcome.subcategory=="1g. Reproduction")
View(reprod_out)
write.csv(reprod_out,paste0(tabledir,today.date,"_1g.csv"))
...
sort(unique(data_all$Intervention.subcategory)) # 32 sub-interventions (#8s were not included)
  # Seems like only 26 sub-interventions were coded for
sort(unique(data_all$Outcome.subcategory)) # 42 sub-outcomes (bc we added in 1c. Catch/Yield)
  # Seems like only 40 sub-interventions were coded for
...
out_sub_list$out_sub[!out_sub_list$out_sub%in%data_all$Outcome.subcategory] # 2 sub-outcomes. Correct: 42-40=2
...
mutate(coral=ifelse(grepl("Coral",Habitat.type,ignore.case = T),1,0),
       seagrass=ifelse(grepl("Seagrass",Habitat.type,ignore.case = T),1,0), # note: ignores case, so the lowercase "seagrass" is fine!
...
# ADJUSTING 1c. Yield/CPUE to be in category 4... 
  # not sure exactly what we want to do with it, but it will at least work better for Austin for the time-being
# Adjust 1c. Yield/CPUE as an intervention entirely, and adjust numbers
data_all <-data_all %>% 
  mutate_at(vars(Outcome.subcategory), ~gsub("^1c","4e",.)) %>% # Moving "1c. Yield/CPUE"to be "4e. Yield/CPUE" and then adjusting all other 1 #s
  mutate_at(vars(Outcome.subcategory), ~gsub("^1d","1c",.)) %>% 
  mutate_at(vars(Outcome.subcategory), ~gsub("^1e","1d",.)) %>%
  mutate_at(vars(Outcome.subcategory), ~gsub("^1f","1e",.)) %>% 
  mutate_at(vars(Outcome.subcategory), ~gsub("^1g","1f",.)) %>% 
  mutate_at(vars(Outcome.subcategory), ~gsub("^1h","1g",.)) %>% 
  mutate_at(vars(Outcome.subcategory), ~gsub("^1i","1h",.)) %>% 
  mutate_at(vars(Outcome.subcategory), ~gsub("^1j","1i",.)) %>% 
  mutate_at(vars(Outcome.subcategory), ~gsub("^1k","1j",.)) %>% 
  mutate_at(vars(Outcome.subcategory), ~gsub("^1l","1k",.)) 
...
sort(unique(data_all$Outcome.subcategory)) # 40 sub-outcomes coded, no.1-7. with "4e. Yield/CPUE"
...
###---- NEW!! ----
# new packages to install for new worldmap (bring to the top if these work)
...
###---- KEEP this the SAME!!! Just making tweaks based on new data set----
unique(data_all$study.ctry)
  # fix data in country column
  # note: added more countries to this list in 2023 code
ctry.fix <- data_all %>%
  mutate(study.ctry=gsub(",",";",study.ctry),  # replace "," with ";" to help with splitting
         study.ctry=gsub("Korea; South","Korea, South",study.ctry), #first is old (in our dropdowns), second matches map countries
         ...
         study.ctry=gsub("Vietnam","Viet Nam",study.ctry),
...
head(ctry.all)
  #Note: the NA country is AID 705 Campbell et al. - I emailed the corresponding author, waiting to hear back!
...
ctry_sum$Country[!ctry_sum$Country%in%country$Country] #this is identifying countries that DON'T fit into the list
#agrep("NA",country$Country, value = T) - search original list
...
### I believe I used SF in my class this summer! Come back to this!!!

####THIS IS where I'm in the process of changing to sf!!
# ----- NEW ------
 ### LOOKING FOR A MAP THAT CONTAINS France (Reunion) and France (Mayotte)
# following https://r-spatial.org/r/2018/10/25/ggplot2-sf.html 

world <- ne_countries(scale = "medium", returnclass = "sf") # world has 240 entries, no Reunion no Mayotte
large.world <- ne_countries(scale = "large", returnclass = "sf") # large world has 240 entries, no Reunion no Mayotte




### THIS is the change I haven't gotten to yet!!!
#------ OLD TO TWEAK -----

# read in map
map <- rgdal::readOGR(paste0(mapdir,"TM_WORLD_BORDERS-0.3/TM_WORLD_BORDERS-0.3.shp")) #rgdal was taken from R! Need to use SF or Terra!!! 
...
data_all %>%
  ...
  summarise(num=n_distinct(aid), pct=num/num.studies*100)  %>%
  arrange (desc(num))  # this arranges them in descending order 
### NOTE: These are messy, need to make some edits here!
...
# spot checks
  # table(test$n.int) # unsure what this does
  # filter(test,n.int==3) # unsure what this does
...
# Studies with multiple conservation interventions
97/208*100
# David: how do I ask mulit.int.studies to count the number of distinct aids 
# and then give me the percent out of total num.studies? tried, cant!
...
```
## Bass Screening Round 2: For Ease
1. Filter OUT Interdisc outcome papers
2. Remove obvious fisheries papers
3. Remove (by me, by highlights in the file) any fisheries papers

```{r eval=T}


# 1. Filter OUT Interdisc outcome papers
  # Non_Interdisc Outcomes and export csv
Non_Interdisc_Out <- data_all %>%
  filter(interdisc.aid == 0)
# View(select(Non_Interdisc_Out,Outcome_cat,eco.aid:interdisc.aid)) # spot check
write.csv(Non_Interdisc_Out,paste0(tabledir,today.date,"_Non_Interdisc_Outcomes.csv"))

# 2. Remove obvious fisheries papers

unique(data_all$Intervention.subcategory)
fish.interventions <- c("2b. Fisheries management")

data_all <- data_all %>%
  # select(aid, Outcome_cat) %>%
  mutate(fish.int=ifelse(Intervention.subcategory%in%fish.interventions,1,0)) %>% 
  group_by(aid) %>% 
  mutate(fish.aid=max(fish.int),
         fish.interdisc.aid=ifelse(fish.aid==1 | interdisc.aid==1,1,0)) 

# Which studies have both Fisheries Management interventions?
n_distinct(data_all$aid[data_all$fish.interdisc.aid == 1])

  # no 2b. Fisheries management interventions and Non_Interdisc Outcomes with and export csv
No_Fish_Int_Non_Interdisc_Out <- data_all %>%
  filter(fish.interdisc.aid == 0)
# View(select(Non_Interdisc_Out,Outcome_cat,eco.aid:interdisc.aid)) # spot check
write.csv(No_Fish_Int_Non_Interdisc_Out,paste0(tabledir,today.date,"_Non_Fish_Int_Non_Interdisc_Outcomes.csv"))
